...
{ 
	...
	ei_linked_point_t *cadre=rounded_frame(*(widget->content_rect),0,0);
	...
	if (frame->border_width != 0){
		...
		ei_linked_point_t *cadre_inferieur_bas=rounded_frame(
			widget->screen_location, 0, -1);
		ei_linked_point_t *cadre_inferieur_haut=rounded_frame(
			widget->screen_location, 0, 1);
		...
		if (frame->relief ==1){
			signe = 1;
		}else if  (frame->relief ==2){
			signe= -1;
		}else{
			signe=0;
		}
		ei_color_t color_sup= eclaircir_assombrir(frame->color,
							  50, signe);
		ei_color_t color_inf=eclaircir_assombrir(frame->color, 
							 50, -signe);


		ei_draw_polygon(surface, cadre_inferieur_bas, color_inf, 
				clipper);

		ei_draw_polygon(surface, cadre_inferieur_haut, color_sup, 
				clipper);

		ei_draw_polygon(pick_surface, 
				cadre_inferieur_bas,*(widget->pick_color), 
				clipper);

		ei_draw_polygon(pick_surface, 
				cadre_inferieur_haut,*(widget->pick_color), 
				clipper);
	...
	ei_draw_polygon(surface, cadre, frame->color, clipper);
...
void frame_geomnotifyfunc(struct ei_widget_t *widget, ei_rect_t rect)
{
	...
	if (widget != NULL){
		...
		widget->content_rect->top_left.x=
			widget->screen_location.top_left.x+b;
		widget->content_rect->top_left.y=
			widget->screen_location.top_left.y+b;
		widget->content_rect->size.width=
			widget->screen_location.size.width-2*b;
		widget->content_rect->size.height=
			widget->screen_location.size.height-2*b;
	}
}
...
/* Trace un arc de points: c:centre/r:rayon/a1:angle de début/a2:angle final*/
/* Requiert a1 < a2, r >= 0, et tail != NULL */
/* Insert l'arc à la suite de head (change sa valeur) */
/* Renvoie la queue de liste */
ei_linked_point_t* create_arc(ei_point_t c, uint32_t r, float a1, float a2, 
...
ei_linked_point_t* rounded_frame(ei_rect_t rect, int radius, int8_t mode)
{
        ...
        if (mode >= 0){
                ...
                cour = create_arc(c, radius, 3.0*M_PI/2.0, 7.0*M_PI/4.0, 
				  &(cour->next));
        }
        if (mode <= 0){
                ei_point_t c;
                ...
                c.y = rect.top_left.y + radius;
                if (pts == NULL){
                        cour = create_arc(c, radius, 7.0*M_PI/4.0, 2.0*M_PI, 
					  &pts);
                }
                else {
                        cour = create_arc(c, radius, 7.0*M_PI/4.0, 2.0*M_PI, 
                ...
                c.x = rect.top_left.x + radius;
                cour = create_arc(c, radius, M_PI/2.0, 3.0*M_PI/4.0, 
				  &(cour->next));
        }
        if (mode != 0){
                ...
                if (rect.size.width >= rect.size.height) {
                        ...
                        h2->point.x = rect.top_left.x + rect.size.width 
				- rect.size.height/2;
                }
                else {
                        ...
                        h1->point.y = rect.top_left.y + rect.size.height 
				- rect.size.width/2;
                }
                if (mode > 0){
                        cour->next = h2;
		     ...
		     ei_rect_t*		clipper)
{
        ...
        ei_button_t* button = (ei_button_t*) widget;
        ei_linked_point_t* relief_sup = rounded_frame(widget->screen_location, 
						      button->corner_radius,1);
        ei_linked_point_t* relief_inf = rounded_frame(widget->screen_location, 
						      button->corner_radius,-1);
        int rayon_corps = button->corner_radius - button->border_width;
        if (rayon_corps<0){
                rayon_corps = 0;
        }
        ei_linked_point_t* corps = rounded_frame(*(widget->content_rect), 
						 rayon_corps, 0);
        ...
        hw_surface_lock(surface);
        hw_surface_lock(pick_surface);
        ei_draw_polygon(surface, relief_sup,

			eclaircir_assombrir(button->color,50,signe), clipper);
        ei_draw_polygon(surface, relief_inf, 
			eclaircir_assombrir(button->color,50,-signe), clipper);
        ei_draw_polygon(surface, corps, button->color,  clipper);

        /* On trace aussi dans pick_surface */
        ei_draw_polygon(pick_surface, relief_inf, *(widget->pick_color), 
			clipper);
        ei_draw_polygon(pick_surface, relief_sup, *(widget->pick_color), 
			clipper);
...
void button_geomnotifyfunc(ei_widget_t *widget, ei_rect_t rect)
{
        if (widget != NULL){
			...
			/* Le content_rect pointe vers la screen_location, il
                ...
                widget->content_rect->top_left.x = rect.top_left.x 
			+ button->border_width;
                widget->content_rect->top_left.y = rect.top_left.y 
			+ button->border_width;
                widget->content_rect->size.width = rect.size.width 
			- 2*button->border_width;
                widget->content_rect->size.height = rect.size.height 
			- 2*button->border_width;
        }
}
                       ...
                       ei_rect_t*		clipper)
{
        ...
        ei_point_t centre1 = ei_point(widget->screen_location.top_left.x + 
				      radius_header, 
				      widget->screen_location.top_left.y + 
				      radius_header);

        ei_point_t centre2 = ei_point(widget->screen_location.top_left.x 
        ...
        cour = create_arc(centre2, radius_header, 3.0*M_PI/2, 2*M_PI, 
			  &(cour->next));
        ...
        ei_point_t title_top_left = ei_point(widget->screen_location.top_left.x 
					     + 2 * radius_header,
                                             widget->screen_location.top_left.y);
...
void toplevel_setdefaultsfunc(struct ei_widget_t* widget)
{
	...
	widget->content_rect->size.height = widget->requested_size.height;
		...
		+ toplevel->border_width * 2;
        widget->screen_location.size.height = widget->requested_size.height 
		+ toplevel->height_header + toplevel->border_width;
}
